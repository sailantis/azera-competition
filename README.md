...
- **CakePHP accumulates state in a resident worker** — the real RoadRunner worker reaches 40.6 MB at its worst endpoint, against 1.7–9.0 MB for the other five frameworks. CakePHP is _not_ the heaviest to boot — it has the **lightest** resident footprint of all six (0.85 MB) — so this is accumulation, not footprint, and it is why the resident-memory chart draws each framework as a _range_: the left cap is the boot footprint, the dot the heap after the last endpoint, and the right cap the worst endpoint. A single bar could only show one of those. The growth is roughly linear in endpoints served (≈1.6 MB per endpoint), the logger / DB layer / framework registries were each ruled out, and a per-request `gc_collect_cycles()` reduces it. We have not root-caused it to a specific CakePHP structure. The real-RoadRunner view is therefore the one to read for CakePHP memory; treat its right cap as an upper bound and its left cap as authoritative.
