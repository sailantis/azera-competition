...
- **CakePHP accumulates state in a resident worker** — the real RoadRunner worker retains 39.7 MB by the end of the endpoint suite, against 0.74–1.74 MB for the other five frameworks. CakePHP is *not* the heaviest to boot — it has the **lightest** resident footprint of all six (0.85 MB) — so this is accumulation, not footprint, and it is why the resident-memory chart shows both panels instead of one bar. The growth is roughly linear in endpoints served (≈1.6 MB per endpoint), the logger / DB layer / framework registries were each ruled out, and a per-request `gc_collect_cycles()` reduces it. We have not root-caused it to a specific CakePHP structure. The real-RoadRunner view is therefore the one to read for CakePHP memory; treat its growth panel as an upper bound and its footprint panel as authoritative.
