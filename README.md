...
- **Resident-worker memory is our instrumentation, not the framework's** — every adapter records one entry per executed query into a `DbEventLog`. In a process that stays alive (the warm/RoadRunner modes) that store must be wiped per request, or the harness's own bookkeeping grows without bound and the memory chart reports *it* instead of the framework. Azera and CodeIgniter always did this; Laravel, Symfony and Spiral did not until the `Benchmark apps: wipe the DB event log per request` commit, which had inflated their published warm peak by up to 5× (Symfony 42 MB → 6 MB, Laravel 40 MB → 8 MB). If you add an adapter, make your log request-scoped.
- **CakePHP residual growth** — the same probe still shows CakePHP retaining ~0.18 KB per request in a resident worker (measured over 5,000 requests on every endpoint, including ones that touch no database). The logger, the DB layer and the framework's registries were each ruled out; the growth lives in CakePHP's long-lived `Server`/middleware pipeline when the `Server` instance is reused across requests, and a per-request `gc_collect_cycles()` reduces it. It is a small, real effect that we have not root-caused to a specific CakePHP structure — read the CakePHP memory bar as an upper bound.
