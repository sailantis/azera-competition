...
- **Routing** (`GET /`): Azera at 0.016ms median, x 8.3 faster than CakePHP.
- **ORM / Active Record** (`GET /items`): Azera at 0.133ms median, x 3.6 faster than CakePHP.
- **Query Builder** (`GET /items-qb`): Azera at 0.088ms median, x 3.1 faster than CakePHP.
- **REST API (JSON)** (`GET /api/items`): Azera at 0.038ms median, x 6.6 faster than CakePHP.
- **AOP (Aspect-Oriented)** (`GET /features/aop`): Azera at 0.125ms median, x 5.8 faster than Symfony.
- **Cache** (`GET /features/cache`): Azera at 0.014ms median, x 7.4 faster than CakePHP.
- **Database Events** (`GET /features/db-events`): Azera at 0.047ms median, x 5.4 faster than CakePHP.
- **Event Dispatcher** (`GET /features/events`): Azera at 0.038ms median, x 3.8 faster than CakePHP.
- **Validation** (`GET /features/validation`): Azera at 0.018ms median, x 10.0 faster than CakePHP.
- **Config** (`GET /features/config`): Azera at 0.010ms median, x 9.3 faster than CakePHP.
- **Request-Scoped Services** (`GET /features/request-scoped`): Azera at 0.010ms median, x 9.5 faster than CakePHP.
